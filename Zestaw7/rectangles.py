...
class Rectangle:
    ...
    def area(self):             # pole powierzchni
        a = math.sqrt(math.pow(self.pt2.x - self.pt1.x, 2) +
                      math.pow(self.pt1.y - self.pt1.y, 2))
        b = math.sqrt(math.pow(self.pt2.x - self.pt2.x, 2) +
                      math.pow(self.pt2.y - self.pt1.y, 2))
        return a * b
    ...
    def intersection(self, other):      # część wspólna prostokątów
        if self.pt2.x < other.pt1.x or self.pt1.x > other.pt2.x:
            # brak czesci wspolnej dla tych wspolrzednych
...
class TestRectangle(unittest.TestCase):
    ...
    def test_print(self):
        ...
        self.assertEqual(Rectangle(3, 5, 6, 8).__repr__(),
                         "Rectangle(3, 5), (6, 8)")
        self.assertEqual(Rectangle(4, 2, 5, 7).__repr__(),
                         "Rectangle(4, 2), (5, 7)")
    ...
    def test_move(self):
        self.assertEqual(Rectangle(5, 4, 6, 7).move(
            4, 3), Rectangle(9, 7, 10, 10))
        self.assertEqual(Rectangle(2, 1, 4, 8).move(
            1, 2), Rectangle(3, 3, 5, 10))

    def test_intersection(self):
        self.assertEqual(Rectangle(2, 2, 7, 5).intersection(
            Rectangle(4, 1, 9, 4)), Rectangle(4, 2, 7, 4))
        self.assertEqual(Rectangle(4, 1, 9, 4).intersection(
            Rectangle(2, 2, 7, 5)), Rectangle(4, 2, 7, 4))
        self.assertEqual(Rectangle(2, 2, 5, 4).intersection(
            Rectangle(6, 4, 9, 6)), Rectangle(0, 0, 0, 0))

    def test_cover(self):
        self.assertEqual(
            Rectangle(-2, -4, 6, 7).cover(Rectangle(-2, -4, 4, 3)), Rectangle(-2, -4, 6, 7))
        self.assertEqual(Rectangle(0, 0, 3, 3).cover(
            Rectangle(2, 2, 4, 4)), Rectangle(0, 0, 4, 4))
        self.assertEqual(
            Rectangle(-2, -3, 4, 1).cover(Rectangle(0, -1, 1, 0)), Rectangle(-2, -3, 4, 1))

    def test_make4(self):
        self.assertEqual([Rectangle(0, 0, 2, 2), Rectangle(2, 0, 4, 2), Rectangle(
            2, 2, 4, 4), Rectangle(0, 2, 2, 4)], Rectangle(0, 0, 4, 4).make4())
